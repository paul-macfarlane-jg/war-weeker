/**
 * Captures Awards and FAQ screenshots (ticket 13): /xi/awards, /xi/faq and
 * the More tab on phone (and Awards on desktop), plus the admin Awards list
 * and new-Award form. Screenshots land in
 * test-results/13-awards-and-faq/.
 *
 ...
 *   pnpm tsx scripts/awards-evidence.ts
 */
...
const OUT = path.resolve(process.cwd(), "test-results/13-awards-and-faq");
const AUTH_SECRET = `evidence-only-secret-${randomUUID()}`;
const EMAIL = "smoke-organizer@jahnelgroup.com";

const SHOTS = [
  { file: "phone-awards.png", target: "/xi/awards", mobile: true },
  { file: "desktop-awards.png", target: "/xi/awards", mobile: false },
  { file: "phone-faq.png", target: "/xi/faq", mobile: true },
  { file: "phone-more.png", target: "/xi/more", mobile: true },
  { file: "desktop-admin-awards.png", target: "/admin/awards", mobile: false },
  {
    file: "desktop-admin-new-award.png",
...
/**
 * The evidence session needs to be an Organizer to see /admin/awards,
...
async function main() {
  ...
  const dir = mkdtempSync(path.join(os.tmpdir(), "awards-evidence-"));
