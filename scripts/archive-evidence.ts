/**
 * Captures Archive screenshots (ticket 11): `/history` and past War Weeks
 * in their own Appearance Themes, on desktop and phone widths. Screenshots
 * land in test-results/11-archive/.
 *
 ...
 *   pnpm tsx scripts/archive-evidence.ts
 */
...
const OUT = path.resolve(process.cwd(), "test-results/11-archive");
const AUTH_SECRET = `evidence-only-secret-${randomUUID()}`;
const EMAIL = "smoke-evidence@jahnelgroup.com";

const SHOTS = [
  { file: "desktop-history.png", target: "/history", mobile: false },
  { file: "phone-history.png", target: "/history", mobile: true },
  { file: "desktop-viii-2023.png", target: "/viii", mobile: false },
  { file: "phone-viii-2023.png", target: "/viii", mobile: true },
  { file: "desktop-x-2025.png", target: "/x", mobile: false },
  { file: "phone-i-2016-link-only.png", target: "/i", mobile: true },
  { file: "desktop-ii-2017-link-only.png", target: "/ii", mobile: false },
];
...
async function main() {
  ...
  const dir = mkdtempSync(path.join(os.tmpdir(), "archive-evidence-"));
