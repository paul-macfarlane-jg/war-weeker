/**
 * Evidence for D3 (the Organizer guide at `/admin/guide`): full-page
 * screenshots at 375px and 1280px, plus a horizontal-overflow sweep at
 * 375/768/1280px. Screenshots land in test-results/admin-guide/.
 *
 * Needs a production build, the seeded local Postgres (run `pnpm seed:all`
 * first), and Google Chrome. Starts its own server on port 3313:
 *   pnpm tsx scripts/admin-guide-evidence.ts
 */
...
const OUT = path.resolve(process.cwd(), "test-results/admin-guide");
const AUTH_SECRET = `evidence-only-secret-${randomUUID()}`;
const EMAIL = "evidence-ap-admin-guide@jahnelgroup.com";
...
async function main() {
  ...
  const dir = mkdtempSync(path.join(os.tmpdir(), "admin-guide-evidence-"));
  ...
  try {
    ...
    const captureFullPage = async (name: string, width: number) => {
      const height = await evaluate<number>(
    ...
    // Overflow sweep at 375/768/1280px for /admin/guide.
    const lines: string[] = [];
    for (const width of [375, 768, 1280]) {
      await size(width, 900);
      await open("/admin/guide");
      const px = await overflow();
      lines.push(`/admin/guide @${width}: ${px}`);
      console.log(`/admin/guide @${width}: horizontal overflow ${px}px`);
    }
    writeFileSync(path.join(OUT, "overflow.txt"), lines.join("\n") + "\n");
    if (lines.some((l) => !l.endsWith(": 0"))) process.exitCode = 1;

    // Full-page screenshots at 375px (phone) and 1280px (desktop).
    await size(375, 812);
    await open("/admin/guide");
    await captureFullPage("guide-375", 375);

    await size(1280, 900);
    await open("/admin/guide");
    await captureFullPage("guide-1280", 1280);
